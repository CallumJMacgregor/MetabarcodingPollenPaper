...
rm(list=ls())
j <- c("reshape2","ggplot2","rmarkdown","plyr","bipartite","vegan","lme4","glmmADMB","MASS","car","scales","AICcmodavg","effects","gridExtra")
...
```{r load_function}
f <- c("SpecAccumFunctions.R","CheckResidsFunction.R","MultiplotFunction.R","NetworkBootstrapFunction.R")
lapply(f, source)
```
...
Again we can see that there is widespread contamination by _Kalanchoe_ but that _Dracaena_ has not spread. The PCR positive POS4 appears to have failed entirely and it isn't clear that _Crassula helmsii_ is actually POS3.
...
completeness <- weighted.mean(ICgood$completeness, ICgood$chao, na.rm=T)
...
```{r bootstrap_meta}
bs_meta <- bootstrap_robustness(networkframe, 100) # used 1000 for data in manuscript
print(bs_meta)
```
...
micro_completeness <- weighted.mean(micro_ICgood$completeness, micro_ICgood$chao, na.rm=T)
...
```{r bootstrap_micro}
bs_micro <- bootstrap_robustness(micro_networkframe, 100) # used 1000 for data in manuscript
print(bs_micro)
```
...
If anything, these are worse. Probably a better approach is to use a zero-inflation model.

```{r model3zip}
# zero-inflated poisson
model3ZIP <- glmmadmb(IndividualInteractionRichness ~  Method + Treatment
                      + (1|TrapID),
                      zeroInflation=TRUE,
                      family = "poisson",
                      data = merged)
...
Anova(model3ZIP, type="III")

 chkres.zi(model3ZIP, merged$Method, merged$Treatment) # these aren't great either
...
We want to repeat this analysis for pollen-positive moths only, in case the different probability of zeroes we've already demonstrated skews things.
...
```{r model3zip1}
# zero-inflated poisson
model3ZIPP <- glmmadmb(IndividualInteractionRichness ~  Method + Treatment
                      + (1|TrapID),
                      zeroInflation=TRUE,
                      family = "poisson",
                      data = mergeP)
...
Anova(model3ZIPP, type="III")

 chkres.zi(model3ZIPP, mergeP$Method, mergeP$Treatment) # these aren't great either
...
# now create the framework for the plot
newdata5 <- expand.grid(Method=c("Metabarcoding","Microscopy"),PollenSpecies=1)
...
# now create the framework for the plot
newdata6 <- expand.grid(Method=c("Metabarcoding","Microscopy"),chao=1)
...
bs_meta_fam <- bootstrap_robustness(networkframe_fam, 100) # used 1000 for data in manuscript
print(bs_meta_fam)
```
...
bs_micro_fam <- bootstrap_robustness(micro_networkframe_fam, 100) # used 1000 for data in manuscript
print(bs_micro_fam)
```
...
```{r multinetwork}
m2 <- grid.arrange(g2.1,g2.2,g2.3,g2.4,g2.5,g2.6,ncol=2)

ggsave("Fig3.svg", plot = m2, device = "svg", path = "Plots", width = 290, height = 320, units = "mm")
```
